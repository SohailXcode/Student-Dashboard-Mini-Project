  ...
  const [students, setStudents] = useState(() => {
    try {
      ...
      if (raw) {
        ...
        // normalize older string-array format into objects
        ...
        <div className="form-row">
          {!showAddForm ? (
            ...
            <div className="input-group" style={{width:'100%'}}>
              <input type="text" placeholder="Full name" value={studentName} onChange={(e)=> setStudentName(e.target.value)} aria-label="Full name" />
              <input type="email" placeholder="Email (optional)" value={studentEmail} onChange={(e)=> setStudentEmail(e.target.value)} aria-label="Email" />
              <input type="text" placeholder="Notes (optional)" value={studentNotes} onChange={(e)=> setStudentNotes(e.target.value)} aria-label="Notes" />
              <button className="btn btn--primary" onClick={addStudent}>Save</button>
              <button className="btn btn--ghost" onClick={()=> { setShowAddForm(false); setStudentName(''); setStudentEmail(''); setStudentNotes(''); }}>Cancel</button>
            </div>
          )}
        </div>
